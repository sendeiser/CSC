                  ...
                  <AnimatePresence initial={false}>
                    {isExpanded && (
                      ...
                      >
                        ...
                        {/* 2. Upload Proof of Payment Box (Comprobante de Pago) */}
                        <div className="bg-gradient-to-r from-emerald-50 to-teal-50/60 p-5 rounded-2xl border border-emerald-200 space-y-3">
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-emerald-800/80">
                                  Adjuntá la captura o comprobante de tu transferencia bancaria para agilizar tu pedido.
                                </p>
                              </div>
                            </div>

                            {order.receipt_url ? (
                              <span className="px-3 py-1 bg-emerald-600 text-white text-xs font-bold rounded-full inline-flex items-center space-x-1 shadow-sm">
                          ...
                          {order.receipt_url ? (
                            <div className="flex flex-wrap items-center justify-between gap-3 pt-1">
                              <div className="flex items-center space-x-3 bg-white p-2 rounded-xl border border-emerald-200 shadow-sm">
                                <img
                                  src={order.receipt_url}
                                  alt="Comprobante de pago"
                                  className="w-12 h-12 rounded-lg object-cover bg-slate-100 border border-slate-200 cursor-pointer"
                                  onClick={() => window.open(order.receipt_url, '_blank')}
                                />
                                <div>
                                  <span className="text-xs font-bold text-slate-800 block">Comprobante Registrado</span>
                                  <a
                              ...
                              <label className="inline-flex items-center space-x-1.5 px-4 py-2 bg-white hover:bg-emerald-100 text-emerald-800 text-xs font-bold rounded-xl border border-emerald-300 shadow-sm transition-all cursor-pointer">
                                <Upload className="w-3.5 h-3.5" />
                                <span>{uploadingOrderId === order.id ? 'Subiendo...' : 'Reemplazar Comprobante'}</span>
                                <input
                                  type="file"
                                  accept="image/*,.pdf"
                                  className="hidden"
                                  ...
                                  onChange={(e) => {
                                    if (e.target.files && e.target.files[0]) {
                                      handleUploadReceipt(order.id, e.target.files[0]);
                                    }
                                  }}
                                />
                              </label>
                            ...
                            <div className="pt-2">
                              <label className="w-full flex flex-col items-center justify-center p-4 border-2 border-dashed border-emerald-300 rounded-2xl bg-white hover:bg-emerald-50/50 transition-all cursor-pointer group">
                                <div className="p-3 bg-emerald-100 text-emerald-700 rounded-full group-hover:scale-110 transition-transform mb-2">
                                  <Upload className="w-5 h-5" />
                                </div>
                                <span className="text-xs sm:text-sm font-bold text-slate-800">
                                  {uploadingOrderId === order.id ? 'Procesando archivo...' : 'Haz clic aquí para seleccionar tu comprobante'}
                                </span>
                                <span className="text-[11px] text-slate-400 mt-0.5">Soporta JPG, PNG, WEBP o PDF</span>
                                <input
                                  type="file"
                                  accept="image/*,.pdf"
                                  className="hidden"
